...
func main() {
	...
	time := strings.Join(strings.Fields(strings.Split(parts[0], ":")[1]), "")
	record := strings.Join(strings.Fields(strings.Split(parts[1], ":")[1]), "")
	races := map[int]int{stringToInt(time): stringToInt(record)}
